...
/*   Updated: 2025/02/04 13:49:42 by alerusso         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	how_many_strings(const char *str, char c, int *s_n, int *i);
...
char	**ft_split(char const *s, char c)
{
	...
	strings = NULL;
	how_many_strings((char *)s, c, &str_num, &i);
...
static int	ft_alloc(char ***strings, const char *s, char c, int s_num)
{
	...
	*strings = (char **)calloc((s_num + 1), sizeof(char *));
	if (*strings == NULL)
		return (42);
	s_num = 0;
	i = 0;
	while (s[i] != '\0')
	{
		...
		while ((s[i] != '\0') && (s[i++] != c))
			++str_len;
		if (str_len != 0)
			(*strings)[s_num] = (char *)calloc((str_len + 1), sizeof(char));
